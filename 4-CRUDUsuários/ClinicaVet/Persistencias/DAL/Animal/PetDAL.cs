...
namespace Persistencia.DAL.Animal
{
    public class PetDAL
    {
        ...
        {
            return context.Pets.Include(c => c.Especie).Include(c => c.Cliente).OrderBy(b => b.Nome);
        }

        public Pet ObterPetPorId(long id)
        {
            return context.Pets.Where(p => p.PetId == id).Include(f => f.Especie).Include(c => c.Cliente).First();
        }
